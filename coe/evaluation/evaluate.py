"""CLI: run dataset evaluation (``python -m coe.evaluation.evaluate``)."""
...
from coe.config import COEConfig, _parse_override_value, get_default_config_path
from coe.evaluation.datasets import load_dataset_any
from coe.evaluation.runner import DatasetEvaluator
...
def split_eval_overrides(
    ...
    """Separate evaluation kwargs from COE config overrides."""
...
def _write_resolved_config_to_output(output_dir: Path, cfg: COEConfig) -> None:
    """Persist the resolved COE config (after env + overrides) for reproducibility."""
...
def main(argv: Optional[List[str]] = None) -> int:
    parser = argparse.ArgumentParser(
        description="Run COE on a dataset and write metrics (JSONL log + metrics.json).",
    )
    parser.add_argument(
        "--config",
        type=str,
    ...
    parser.add_argument(
        ...
        help=(
            ...
            "prediction_column) plus COE config keys (e.g. llm.model_name=..., search.strategy=mcts). "
    ...
    system = COESystem(config_path=args.config, config_overrides=config_overrides)
