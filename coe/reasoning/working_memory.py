...
from coe.llm.roles import Relation
from coe.reasoning.generator import merge_logs, _PRUNE_TRIPLES_BATCH_SIZE
from coe.retrieval.entity_linking import link_entities_azure, link_entities_llm
from coe.retrieval.wikidata import (
    WikiTriple,
    WikidataClient,
    WikidataEntity,
    WikidataProperty,
...
async def parse_graph_from_text(
    ...
    from coe.llm.roles import RELATION_EXTRACTOR, RelationExtractionInput, execute_role
...
async def _aprune_triple_strings_llm(
    ...
    from coe.llm.roles import TRIPLE_PRUNER, TriplePruneInput, execute_role
...
class _MemoryFormatter:
    ...
    def format_item(content: str, provenance) -> str:
        from coe.llm.roles import SourceType
...
class WorkingMemory:
    ...
    async def _arun_consolidation(self, client, question: str, raw_memory: str, interaction_memory=None):
        from coe.llm.roles import MEMORY_CONSOLIDATOR, MemoryConsolidationInput, MemoryConsolidationOutput, MemoryItem, SourceType, execute_role
    ...
    def _textualize_graph_to_text(self) -> List[str]:
        ...
        from coe.utils.graph import textualize_graph
    ...
    def add_node_to_graph_memory(self, node):
        from coe.llm.roles import Entity as OpenIEEntity
        from coe.utils.graph import get_node_id
    ...
    def add_edge_to_graph_memory(
        ...
        *,
        ...
        from coe.llm.roles import Entity as OpenIEEntity, Relation as OpenIERelation
        from coe.utils.graph import get_node_id
