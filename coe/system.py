"""COE System - main orchestrator for question answering."""
...
from coe.retrieval.web_search import WebSearchTool
from coe.retrieval.corpus import CorpusRetriever
from coe.retrieval.reranker import Reranker
from coe.retrieval.wikidata import WikidataClient
from coe.reasoning.cot import cot_search, cot_get_answer
from coe.reasoning.mcts import mcts_search, get_answer
from coe.reasoning.memory import WorkingMemory, InteractionMemory
...
class COESystem:
    ...
    def __init__(
        ...
        config: Optional[COEConfig] = None,
    ):
        if config is not None:
            self.cfg = config
        elif config_dict is not None:
            self.cfg = COEConfig.from_dict(config_dict)
        else:
            self.cfg = COEConfig.from_yaml(config_path, config_overrides)
    ...
    def _initialize(self):
        ...
        logger.info("Initializing COE system...")
        ...
        logger.info("COE system initialized.")
    ...
    def answer(
        ...
        if (
            ...
            from coe.retrieval.graph_retriever_client import GraphRetrieverClient
            ...
            from coe.retrieval.freebase_client import FreebaseClient
...
def answer_question(question: str, config_path=None, config_overrides=None, **kwargs) -> str:
    system = COESystem(config_path=config_path, config_overrides=config_overrides)
    try:
