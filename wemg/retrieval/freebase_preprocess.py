...
Usage (with WEMGConfig-style CLI overrides):
    conda run -n wemg python -m wemg.retrieval.freebase_preprocess \
        dataset=webqsp_sub \
        output=./cache/webqsp_sub_graph_cache

Optional overrides mirror the WEMGConfig schema, e.g.:
...
async def _run(
    ...
    cfg = WEMGConfig.from_yaml(overrides=config_overrides)
...
def main(argv: Optional[List[str]] = None) -> None:
    ...
    passing the rest to WEMGConfig.
    """
    import argparse
    ...
    parser = argparse.ArgumentParser(
        description="Preprocess Freebase subgraph dataset for WEMG evaluation."
    )
