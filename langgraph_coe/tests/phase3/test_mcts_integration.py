...
def _build_real_config() -> LangGraphCoeConfig:
    ...
    # Reranker stays off; the rollout uses the default identity reranker in
    # ``cot.rerank_context``.
    cfg.reranker.enabled = False
    cfg.reranker.top_k = 4
    # Keep the search cheap and bounded for an integration smoke test: one MCTS
    # iteration with a single-step CoT rollout.
    cfg.search.mcts.max_simulation_depth = 1
    return cfg
...
def _build_real_registry(cfg: LangGraphCoeConfig) -> RoleModelRegistry:
    tier = TierConfig(
        ...
        # 16k matches the Phase 1 / Phase 2 integration budgets.
        max_tokens=16384,
...
def _stub_retrieval(monkeypatch: pytest.MonkeyPatch) -> None:
    ...
    link_stub = _StubLinkEntities({"France": "Q142", "Paris": "Q90", "Eiffel Tower": "Q243"})

    monkeypatch.setattr(cot_mod, "build_kg_search_graph", lambda *_a, **_kw: kg_stub, raising=False)
    monkeypatch.setattr(cot_mod, "build_web_research_graph", lambda *_a, **_kw: web_stub, raising=False)
...
async def test_mcts_graph_runs_against_real_qwen(monkeypatch):
    ...
    assert "sub_qa" in node_types.values(), "expansion must emit at least one SUB_QA node"
    assert "final_answer" in node_types.values(), "expansion must emit a FINAL_ANSWER node"

    # Priors port from NODE_TYPE_PRIOR.
    for node in tree.values():
        nt = _ntype(node)
        if nt == "sub_qa":
            assert node["prior"] == mcts_mod.NODE_TYPE_PRIOR[mcts_mod.MCTSNodeType.SUB_QA]
        elif nt == "final_answer":
            assert node["prior"] == mcts_mod.NODE_TYPE_PRIOR[mcts_mod.MCTSNodeType.FINAL_ANSWER]
    ...
    for node_id in path:
        assert tree[node_id]["visits"] >= 1, "backprop must increment visits along the path"

    # One iteration ran and the loop terminated on the iteration cap.
    assert final["iteration"] == 1

    # ── Memory sync ────────────────────────────────────────────────────────────
    assert final.get("text_memory"), "mem_update must leave non-empty consolidated memory"
...
async def test_mcts_terminal_final_answer_is_evaluated_without_expansion(monkeypatch):
    ...
    final = await graph.ainvoke(
        _initial_state(
            ...
            tree=seeded_tree,
            root_id=root_id,
            text_memory=["France is a country in Western Europe; its capital is Paris."],
        ),
