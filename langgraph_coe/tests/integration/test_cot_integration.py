...
def _build_real_config() -> LangGraphCoeConfig:
    ...
    # Reranker stays off; the integration uses the identity reranker in
    # ``cot.rerank_context`` (no reranker server in this stub test). top_k and all
    # other reranker/search params come from config.yaml.
...
async def test_cot_graph_respects_depth_limit_against_real_qwen(monkeypatch):
    ...
    web_stub = _StubWebGraph(
        results=[{"title": "u", "url": "u", "snippet": "u", "full_text": ""}]
    )
    ...
    assert final.get("final_answer"), (
        "depth-limited CoT must still produce a final answer"
    )
