...
def _init_web_search_with_cache(
    monkeypatch, fake_redis, *, provider_results: List[Dict[str, str]]
):
...
async def test_wd_and_web_keys_coexist_in_db1(monkeypatch, fake_redis):
    ...
    assert any(k.startswith("wd:") for k in keys), (
        "wd:* key disappeared after web write"
    )
    assert any(k.startswith("web:") for k in keys), (
        "web:* key must coexist with wd:* in db=1"
    )
