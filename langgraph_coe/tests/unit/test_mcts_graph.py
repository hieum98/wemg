...
def _subq_out(
    *subquestions: str, answerable: bool = False
...
class RoleExecutorSpy:
    ...
    async def __call__(
        ...
        if role.name == "self_corrector":
            text = (
                self._self_corrections.pop(0)
                if self._self_corrections
                else "Corrected."
            )
        ...
        if role.name == "verifier":
            ...
            for i, _item in enumerate(inputs):
                rating = (
                    self._verifier_ratings.pop(0) if self._verifier_ratings else 5.0
                )
                reason = (
                    self._verifier_reasons.pop(0)
                    if self._verifier_reasons
                    else f"view {i}"
                )
...
def _install_graph_spies(
    ...
    *,
    ...
    monkeypatch.setattr(
        mcts_mod, "build_cot_graph", lambda *_a, **_kw: cot_graph, raising=False
    )
    monkeypatch.setattr(
        mcts_mod,
...
def _nodes_of_type(
    tree: Dict[str, Dict[str, Any]], node_type: str
) -> List[Dict[str, Any]]:
