...
async def test_triple_search_agent_still_uses_react_agent_with_fetch_tool(
    ...
    def _fake_create_agent(model, *, tools=None, system_prompt=None, name=None, **kw):
        ...
        return make_fake_react_agent(
            [
                tool_message(
                    "fetch_and_prune_subgraph", ["Berlin -- capital -- Germany"]
                ),
            ]
        )
    ...
    tool_names = [
        getattr(t, "name", str(t)) for t in (create_agent_calls[0]["tools"] or [])
    ]
...
async def test_ner_empty_extraction_skips_link_entities(
    monkeypatch, init_wikidata_tools
):
    ...
    final = await graph.ainvoke(
        {
            "subquery": "nothing here",
            "original_query": "nothing here",
            "context": "",
            "errors": [],
        }
    )
