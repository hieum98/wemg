...
def _open_ie_out(
    *,
    ...
    return roles_mod.OpenIEOutput(
        entities=[
            roles_mod.Entity(id=None, name=name, description=None) for name in entities
        ],
...
class RoleExecutorSpy:
    ...
    def __init__(
        self,
        *,
        consolidation_outputs: Sequence[roles_mod.MemoryConsolidationOutput]
        | None = None,
        ...
        self._consolidation_outputs = list(
            consolidation_outputs or [_memory_out("memory")]
        )
...
async def test_open_ie_consumes_consolidated_memory(monkeypatch):
    ...
    executor = RoleExecutorSpy(
        consolidation_outputs=[
            _memory_out("Only the consolidated memory should be parsed.")
        ],
        relations=[_relation("France", "capital", "Paris")],
        entities=["France", "Paris"],
    )
...
async def test_merge_prunes_only_new_edges_and_does_not_mutate_input_graph(monkeypatch):
    ...
    pruner_text = "\n".join(
        _input_text(v) for v in executor.role_inputs("triple_pruner")
    )
    ...
    assert not _graph_mentions(current_graph, "Lyon"), (
        "input current_graph must not be mutated"
    )
...
async def test_merge_collapses_duplicate_nodes_by_qid(monkeypatch):
    ...
    executor = RoleExecutorSpy(
        consolidation_outputs=[
            _memory_out("France contains Lyon."),
            _memory_out("post"),
        ],
        relations=[
            _relation(
                "France", "contains", "Lyon", subject_id="Q142", object_id="Q456"
            ),
        ],
...
async def test_triple_pruner_batches_new_edges_at_sixteen(monkeypatch):
    ...
    executor = RoleExecutorSpy(
        ...
        relations=relations,
        entities=[f"Subject {i}" for i in range(17)]
        + [f"Object {i}" for i in range(17)],
    )
