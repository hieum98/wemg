...
def test_registry_resolves_web_researcher_to_medium_model():
    """``RoleModelRegistry.get_model('web_researcher')`` returns the *medium* tier instance."""
    cfg = LangGraphCoeConfig.from_yaml()
    registry = RoleModelRegistry(cfg.llm)
    medium = registry.get_model_by_tier("medium")
    resolved = registry.get_model("web_researcher")
    assert resolved is medium, (
        "web_researcher must resolve to the same ChatLiteLLM instance as the medium tier"
    )
