    ...
    async def __call__(
        ...
        if role.name == "extractor":
            if self._extractor_outputs is not None:
                facts = (
                    self._extractor_outputs.pop(0)
                    if self._extractor_outputs
                    else []
                )
                return _extraction_out(*facts), {}
            raw = str(getattr(input_data, "raw_data", "") or "")
            pieces = [chunk.strip() for chunk in raw.split(_EXTRACTOR_BATCH_SEP) if chunk.strip()]
...
def _install_graph_spies(
    ...
    *,
    ...
    web_graph = web_graph or CompiledGraphSpy(
        {"results": [{"title": "Web", "url": "https://example.com", "snippet": "Web ctx", "full_text": ""}]}
    )
    ...
    monkeypatch.setattr(cot_mod, "build_kg_search_graph", lambda *_a, **_kw: kg_graph, raising=False)
    monkeypatch.setattr(cot_mod, "build_web_research_graph", lambda *_a, **_kw: web_graph, raising=False)
    monkeypatch.setattr(
        cot_mod,
...
def test_clear_reducer_uses_typed_sentinel_not_string():
    ...
    assert hasattr(cot_mod, "append_or_clear"), "CoTGraph must export append_or_clear (§7.1)"
...
async def test_retrieval_fanout_invokes_kg_web_and_corpus_per_subquestion(monkeypatch):
    ...
    executor = RoleExecutorSpy(
        subq_outputs=[
            _subq_out(answerable=False, subquestions=subquestions, needs_kg=[True, True]),
...
async def test_web_fanout_disabled_by_default(monkeypatch):
    ...
    executor = RoleExecutorSpy(
        subq_outputs=[
            _subq_out(answerable=False, subquestions=subquestions),  # needs_kg absent → default KG-on
            _subq_out(answerable=True),
        ],
        answers=["France is in Europe.", "Paris is the capital."],
        final_answer="Paris.",
    ...
    assert web_graph.calls == [], "web fan-out must be skipped when web_search.enabled is False"
...
async def test_needs_kg_false_skips_kg_for_that_subquestion(monkeypatch):
    ...
    """
    cot_mod = _import_module()
    subquestions = ["What is the capital of France?", "What is the mechanism of photosynthesis?"]
    executor = RoleExecutorSpy(
        subq_outputs=[
            _subq_out(answerable=False, subquestions=subquestions, needs_kg=[True, False]),
            _subq_out(answerable=True),
        ],
...
async def test_known_entity_overrides_needs_kg_false(monkeypatch):
    ...
    entity_dict = {
        "Q142": roles_mod.WikidataEntity(qid="Q142", label="France", description="country"),
    }
    graph = _build_graph(cot_mod)
...
async def test_answer_generation_receives_reranked_joined_context(monkeypatch):
    ...
    web_graph = CompiledGraphSpy(
        {"results": [{"title": "Web", "url": "u", "snippet": "Web snippet ctx", "full_text": "Web full ctx"}]}
    )
    ...
    async def _rerank(query: Any, contexts: Sequence[str], top_k: int | None = None, **_kw: Any):
...
async def test_depth_limit_routes_to_final_without_retrieval(monkeypatch):
    ...
    cot_mod = _import_module()
    executor = RoleExecutorSpy(
        subq_outputs=[_subq_out(answerable=False, subquestions=["Unasked subquestion"])],
...
async def test_extractor_runs_between_rerank_and_subanswers(monkeypatch):
    ...
    assert extractor_calls, "extractor must be invoked between rerank and gen_subanswers"
...
async def test_extractor_splits_oversized_contexts_into_multiple_batches(monkeypatch):
    ...
    kg_graph = CompiledGraphSpy(
        {"kg_articles": [big_ctx_a, big_ctx_b], "triples": []}
    )
    _install_graph_spies(
        monkeypatch,
        cot_mod,
