...
async def test_kg_search_runs_dont_clobber_visited_qids(monkeypatch, init_wikidata_tools):
    ...
    if "entities" in fields:
        try:
            ...
            inner = next(iter(getattr(fields["entities"].annotation, "__args__", []) or [object]))
    ...
    monkeypatch.setattr(
        kg_mod, "reset_wikidata_session",
        lambda: (_ for _ in ()).throw(AssertionError("KG node must not call reset_wikidata_session")),
        raising=True,
    )
