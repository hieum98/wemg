...
From WSL:
    ...
    pytest langgraph_coe/tests/phase0/test_wikidata_local_integration.py -v
"""
...
def _local_cfg(*, max_hops: int = 3) -> WikidataConfig:
...
async def test_tool_hop_budget_exhausted(local_tools) -> None:
    ...
    assert second and (
        "budget" in second[0].lower() or "hop" in second[0].lower()
    )
