"""Phase 0 §3.4 (db=1) — Wikidata read-through cache target specs.

Plan §3.4 introduces four cache-aware lookup methods on ``WikidataClient`` —
...
``main`` and pass once Phase 0 lands the layer.
"""
...
def _build_cache(redis_client) -> Any:
    """Construct Phase 0's ``RedisDictCache`` (skip cleanly if not yet implemented)."""
    try:
        ...
        try:
            ...
            pytest.skip(
                "Phase 0 §3.4 introduces RedisDictCache (langgraph_coe.tools.cache); "
...
def _build_client(backend, cache) -> WikidataClient:
    """Build a WikidataClient against the Phase 0 ``cache=`` kwarg target."""
    try:
        ...
        pytest.skip(
            "Phase 0 §3.4 introduces ``cache`` kwarg on WikidataClient; "
            "currently exposes only ``redis``."
        )


# ──────────────────────────────────────────────────────────────────────────────
# §3.4 — per-layer keys + TTLs
# ──────────────────────────────────────────────────────────────────────────────
...
async def test_get_entity_miss_writes_key_with_30d_ttl(mini_backend, fake_redis):
    ...
    if not hasattr(client, "get_entity"):
        pytest.skip("WikidataClient.get_entity is a Phase 0 §3.4 addition")
...
async def test_search_entities_writes_key_with_7d_ttl(mini_backend, fake_redis):
    ...
    if not hasattr(client, "search_entities"):
        pytest.skip("WikidataClient.search_entities is a Phase 0 §3.4 addition")
...
async def test_get_triples_writes_key_with_7d_ttl(mini_backend, fake_redis):
    ...
    if not hasattr(client, "get_triples"):
        pytest.skip("WikidataClient.get_triples is a Phase 0 §3.4 addition")
...
async def test_get_wikipedia_content_writes_key_with_30d_ttl(mini_backend, fake_redis):
    ...
    if not hasattr(client, "get_wikipedia_content"):
        pytest.skip("WikidataClient.get_wikipedia_content is a Phase 0 §3.4 addition")

    await client.get_wikipedia_content(QID_BERLIN)

    key = f"wd:enrich:{QID_BERLIN}"
    assert fake_redis.exists(key), f"expected Redis key {key} after get_wikipedia_content"
    ttl = fake_redis.ttl(key)
    assert TTL_30D - 2 <= ttl <= TTL_30D


# ──────────────────────────────────────────────────────────────────────────────
# §3.4 — pruning is NOT cached
# ──────────────────────────────────────────────────────────────────────────────


async def test_pruning_results_are_not_cached(mini_backend, fake_redis):
    """``fetch_and_prune_subgraph`` must not write ``wd:prune:*`` keys (§3.4)."""
    cache = _build_cache(fake_redis)
    client = _build_client(mini_backend, cache)
    if not hasattr(client, "get_triples"):
        pytest.skip("WikidataClient.get_triples is a Phase 0 §3.4 addition")
    ...
    assert prune_keys == [], (
        f"§3.4 forbids caching pruning results; saw keys: {prune_keys!r}"
    )


# ──────────────────────────────────────────────────────────────────────────────
# §3.4 — persistence across client instances on the same db
# ──────────────────────────────────────────────────────────────────────────────
...
async def test_cache_disabled_no_redis_writes(mini_backend, fake_redis):
    ...
    assert list(fake_redis.scan_iter()) == [], (
        "No cache parameter ⇒ no Redis writes"
    )
