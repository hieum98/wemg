...
def _result_payload(url: str, *, title: str = "T", snippet: str = "S", full_text: str = ""):
...
async def test_agent_uses_light_tier_and_web_search_tool(monkeypatch):
    ...
    monkeypatch.setattr(wr_mod, "create_react_agent", _spy, raising=False)
    ...
    graph = wr_mod.build_web_research_graph(registry)
    await graph.ainvoke({
        "subquery": "x", "original_query": "x", "context": "",
    })

    # Bound model is the light-tier sentinel.
    bound_model = captured["kwargs"].get("model") or (captured["args"][0] if captured["args"] else None)
    ...
    if web_researcher is not None:
        prompt = captured["kwargs"].get("prompt") or captured["kwargs"].get("system_prompt")
...
async def test_finalize_dedupes_by_url(monkeypatch):
    ...
    monkeypatch.setattr(
        wr_mod, "create_react_agent",
        lambda *a, **kw: make_fake_react_agent([
            tool_message("web_search", [_result_payload(URL_A), _result_payload(URL_B)]),
...
async def test_agent_invoked_with_recursion_limit(monkeypatch):
    """Layer 1: ``agent.ainvoke(config={'recursion_limit': N})``."""
    wr_mod = _import_module()
    registry = _registry_with_light_model(MagicMock())
    ...
    monkeypatch.setattr(wr_mod, "create_react_agent", lambda *a, **kw: _Agent(), raising=False)

    graph = wr_mod.build_web_research_graph(registry)
    await graph.ainvoke({"subquery": "x", "original_query": "x", "context": ""})

    assert "recursion_limit" in captured_config["config"], (
        "WebResearchGraph must pass recursion_limit to agent.ainvoke (§3.5 layer 1)"
    )
...
async def test_visited_urls_contextvar_dedupes_across_tool_calls(monkeypatch):
    ...
    wr_mod = _import_module()
    if not hasattr(wr_mod, "_cv_visited_urls") and not hasattr(wr_mod, "reset_web_research_session"):
...
async def test_reset_web_research_session_clears_visited_urls():
    ...
    sess = getattr(web_mod, "_get_web_session", None) or getattr(web_mod, "_get_session", None)
    if sess is None:
...
async def test_concurrent_graph_runs_have_isolated_visited_urls(monkeypatch):
    ...
    monkeypatch.setattr(wr_mod, "create_react_agent", _agent_factory, raising=False)
    graph = wr_mod.build_web_research_graph(registry)

    async def _run(label: str):
        return await graph.ainvoke({
            "subquery": label, "original_query": label, "context": "",
        })
...
async def test_provider_failure_recorded_in_errors_not_raised(monkeypatch):
    ...
    monkeypatch.setattr(wr_mod, "create_react_agent", lambda *a, **kw: _BrokenAgent(), raising=False)
